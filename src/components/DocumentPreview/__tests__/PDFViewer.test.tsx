...
jest.mock('react-pdf', () => ({
  Document: ({ children, onLoadSuccess }: any) => {
    React.useEffect(() => { onLoadSuccess && onLoadSuccess({ numPages: 2 }); }, []);
...
describe('PDFViewer', () => {
  ...
  it('loads PDF blob and renders page', async () => {
    ...
    global.fetch = jest.fn(() => Promise.resolve({ ok: true, status: 200, headers: new Headers(), blob: () => Promise.resolve(blob) } as any));
  ...
  it('shows error when response not ok', async () => {
    global.fetch = jest.fn(() => Promise.resolve({ ok: false, status: 404, headers: new Headers(), text: () => Promise.resolve('no') } as any));
